...
通用开发流程
    ...
    impl这里需要注意，传进来的是DTO是为了方便封装前面提交过来的数据；但是为了最终传给mapper，建议用实体类，这里要做对象转换，把DTO转化为实体。如果DTO中有的属性和实体类都有而且属性名一致，可以直接用BeanUtils.copyProperties对象属性拷贝，不用一个一个属性set；至于DTO没有的但是实体有的就需要自行处理了（简单初始化或者像菜品分页查询的categoryName一样涉及到更复杂的多表联查）
